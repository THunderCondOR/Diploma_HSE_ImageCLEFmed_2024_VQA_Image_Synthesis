...
class DiffusionPipeline(BasicModel):
    ...
    def step(self, x_0, condition=None, condition_mask=None):
        ...
        if self.use_self_conditioning:
            with torch.no_grad():
                pred, pred_vertical = noise_estimator(x_t, t, condition, condition_mask, None) 
    ...
    @torch.no_grad()
    def sample(self, num_samples, img_size, prompts=None, guidance_scale=1.0, **kwargs):
        template = torch.zeros((num_samples, 8, *img_size), device=self.device)
