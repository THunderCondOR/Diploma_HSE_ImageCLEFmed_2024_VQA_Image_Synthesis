...
from medical_diffusion.models.utils.attention_blocks import MedfusionAttention, zero_module
...
class SequentialEmb(nn.Sequential):
    def forward(self, input, emb, condition, condition_mask):
        for module in self:
            input = module(input, emb, condition, condition_mask)
        return input
